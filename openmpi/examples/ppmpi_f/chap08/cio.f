C  cio.f -- Collective functions for basic I/O operations
C 
C  Note:  Our basic approach to I/O for Fortran is different
C      from the C approach.  Rather than write general purpose
C      analogs of read and write, we have template functions
C      for I/O.  These need to be modified for use with a
C      particular application.  See functions Cread, 
C      Build_derived_type, and Cprint, below.
C
...
C DEFAULT type for parameters. User updates as needed!
      integer      param1
      real         param2
C User adds additional paramters as needed . . .
      integer      paramN
      include      'mpif.h'
      include      'ciof.h'
      integer      my_io_rank
...
C
      ...
      if (DEBUG) then
         write(6,101) my_io_rank
 101     format(' ','Process ',i2,' > In Cread')
      endif
...
C update parameter list as appropriate!
      call Build_derived_type(N, param1, param2, paramN,
     +      derived_mpi_t)
      if (my_io_rank .EQ.  root) then
          print *, prompt
          read *,  param1, param2, paramN
      endif
      call MPI_BCAST(param1, 1, derived_mpi_t, root, 
     +                  io_comm, ierr)
      Cread = 0   
      return
      end
...
C***********************************************************************
C  TEMPLATE ONLY!! 
C  Here user should add/delete parameters as needed.
...
C
C ******************************************************  
C  TEMPLATE ONLY!! Modify to suit your application.
...
C types & num_params updated by user as appropriate
      integer        param1 , savparm1
      real           param2 , savparm2
      integer        paramN , savparmN
C
      include 'ciof.h'
      include 'mpif.h'
      integer         q
...
C
      ...
      if (DEBUG) then
          write(6,101) my_io_rank
 101      format(' ','Process ',i2,' > In Cprint')
      endif
      retval = Get_io_rank(io_comm,  root)
      if ( retval .EQ. NO_IO_ATTR) then
          Cprint = NO_IO_ATTR
...
C update parameter list as appropriate!
      call Build_derived_type(num_params, param1, param2, 
     +      paramN,
     +      derived_mpi_type)
C
...
C  Copy the output data into io_buf
          call MPI_SEND(param1, 1 , 
     +            derived_mpi_type,
     +            root, 0, io_comm, ierr)              
    
      else    
          do 100 q = 0, root-1
             call MPI_RECV(param1, 1 , derived_mpi_type, 
     +                q, 0, io_comm,  status, ierr)
             print *, ' ',title  
             print *,'Process ',q ,' > ', param1,
     +                  param2, paramN
 100      continue
...
C  Copy the output data into io_buf
          ...
          print *, 'Process',root, ' >', param1, param2,
     +                 paramN
          savparm1 = param1
          savparm2 = param2
          savparmN = paramN
