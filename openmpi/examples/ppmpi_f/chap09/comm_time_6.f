C  comm_time.f  
C  Version 6:  Process 0 starts off the ring pass.
C      Fixed erroneous calc of time_array_order in Initialize.
C      Changed number of message sizes to 2.
C      Removed debug output from Print_results.
C      Corrected calculation of source process rank.
C      Try to check sends and receives using Cprints in loop.
C      Relocate Cprints in loop.
...
C 
C  See Chap 9, pp. 192 & ff and pp. 209 & ff in PPMPI.
C
      PROGRAM Comm6
      INCLUDE 'ciof.h'
      INCLUDE 'mpif.h'
...
C
 ...
 20   format(A, I3, A, I3, A, I3, A, F10.5, A)
...
C ******************************************************  
     ...
     + max_times, min_times,order) 
      ...
      integer i
C
      order = (max_size - min_size)/size_incr +1
C
