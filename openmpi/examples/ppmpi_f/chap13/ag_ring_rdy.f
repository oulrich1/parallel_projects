C  ag_ring_rdy.f -- ring allgather using ready mode sends and
C      blocking receives
C 
...
C 
C  See Chap 13, pp. 306 & ff, in PPMPI.
C
C ******************************************************************
      PROGRAM RingRdy
      INCLUDE 'mpif.h'
...
C
     ...
     +        MOD((my_rank - i - 1 + p) , p)*blocksize
          call MPI_IRECV(y(recv_offset), blocksize,
     +        MPI_REAL , predecessor,
     +        i, ring_comm, request(i), ierr)
 200  continue
C
      call MPI_BARRIER(ring_comm, ierr)
C
      do 300 i = 0 , p - 2
          send_offset = MOD((my_rank - i + p) , p)*blocksize
          call MPI_RSEND(y(send_offset), blocksize, MPI_REAL,
     +        successor, i, ring_comm, ierr)
