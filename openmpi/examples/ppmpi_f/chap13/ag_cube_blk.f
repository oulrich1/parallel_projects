C  ag_cube_blk.f -- hypercube allgather using blocking sends and receives
C 
...
C 
C  See Chap 13, pp. 280 & ff, in PPMPI.
C
C 
C ******************************************************************  
      PROGRAM CubeBlk
...
C
          call MPI_SEND(y(send_offset), 1, hole_type,
     +         partner, 0, comm, ierr)
          call MPI_RECV(y(recv_offset), 1, hole_type,
     +         partner, 0, comm,  status, ierr)
C
          call MPI_TYPE_FREE( hole_type, ierr)    
C  can build new type during next pass   
          eor_bit = eor_bit /2
          and_bits = and_bits/2
 400  continue
C
