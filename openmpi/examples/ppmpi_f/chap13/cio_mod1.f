...
C      operations.  Modififed for use with the allgather
C      programs
C
...
C
      call MPI_COMM_RANK(io_comm,  my_io_rank, ierr)
C   the following is to preserve our original data during receive
      do 50 i = 1, num_params
          param1(i) = param1in(i)
 50   continue
C
      retval = Get_io_rank(io_comm,  root)
