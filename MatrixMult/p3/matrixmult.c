 /* 
    ...
    To run, use mpirun:
        ...
        mpirun -hostfile 4hosts_2procs ./matrixmult < tests/tr4800.out

        mpirun -n 4 ./matrixmult < tr1600.in
...
/* just outputs the hostname so that i can
    confirm this program actually ran on 
    thelab machines.. */
#define OUTPUT_HOSTNAME         false
...
{       
    ...
    if (A != NULL && C != NULL) {
        ...
        // scatter A into other processes
        ...
        for (int i = 0; i < proc_count; ++i) {
            if (my_rank == i) {
                printf("My Rank: %d Scatter took time=%lf\n", my_rank, time_local);
                printf("-----------------------\n");
                MPI_Barrier(MPI_COMM_WORLD);  
            } else {
                MPI_Barrier(MPI_COMM_WORLD); 
