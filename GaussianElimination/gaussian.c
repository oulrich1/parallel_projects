...
#define MAX_VECTOR_VALUE  1.0e10
#endif
...
void gauss_reduce_partial_pivot_parallel(MatrixAugmented *augmat, int thread_count) {
    ...
    /* for local parallelization of finding greatest */ 
    // double *greatest_values = (double*) calloc (thread_count, sizeof(double));
    // int    *greatest_val_row_ids =(int*) calloc (thread_count, sizeof(int));
    ...
    for (pivot = 0; pivot < min(aug_col_size, aug_row_size); ++pivot) { 
            ...
            private(row, multiplier, k) 
        {
            ...
            /* this actually doesnt matter since rows are independant equations */
            // #pragma omp barrier
...
/* performs a backward substitution row reduction
   ...
   has been 'discovered' */
void row_reduce(MatrixAugmented* augmat, int thread_count){
...
{       
    ...
    row_reduce(&augmat, number_of_threads);
    start_time = omp_get_wtime() - start_time;
    printf("Row Reduced Echelon (Time)                 =   %lf\n", start_time);
 ...
 // matrix_print(&augmat.mat);
    ...
    matrix_free(x_result);
    matrix_free(x_transposed);
    matrix_free(b_result);
...
// #pragma omp parallel shared(A, row, pivot, greatest_value, greatest_val_row_id) 
//                      private(local_great_val, local_great_val_row_id)
// {
//     #pragma omp for
//     for (row = pivot + 1; row < aug_col_size; ++row) {
...
/* Creating threads here to parallelize greatest value  
    turns out to be significantly slower.. */
// start_time = omp_get_wtime();
// #pragma omp parallel num_threads(thread_count) 
//     shared(greatest_values, greatest_val_row_ids, pivot, aug_col_size, mat) 
//     private(row)
// {
//     int my_rank = omp_get_thread_num();
//     greatest_values[my_rank]      = mat->data[pivot][pivot];
//     greatest_val_row_ids[my_rank] = pivot;
//     #pragma omp for
//     /* only modifies the array of greatest values */
//     for (row = pivot + 1; row < aug_col_size; ++row) {
//         if (labs(greatest_values[my_rank]) < labs(mat->data[row][pivot])) { 
//             greatest_values[my_rank] = mat->data[row][pivot];
//             greatest_val_row_ids[my_rank] = row;
//         }
//     }
// }
// /* final iteration over greatest values to find THE greatest */
//  // should never be zero anyways, that means its done
// greatest_value = mat->data[pivot][pivot];
// greatest_val_row_id = pivot; //
// for (int i = 0; i < thread_count; ++i) {
//     if (labs(greatest_values[i]) > labs(greatest_value)) { 
//             greatest_value = greatest_values[i];
//             greatest_val_row_id = greatest_val_row_ids[i];
//     }
// }
// time_diff = omp_get_wtime() - start_time;
// time_sum += time_diff;
